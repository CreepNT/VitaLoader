...
public class DefaultElfProgramBuilder extends ElfProgramBuilder {
	...
	protected void adjustSegmentAndSectionFileAllocations(
	...
	protected boolean isZeroFilledFileRegion(ByteProvider byteProvider, long start, long length)
			throws IOException {
		int bufSize = 16 * 1024;
		if (length < bufSize) {
			bufSize = (int) length;
	...
	protected boolean isZeroedArray(byte[] bytes, int len) {
		for (int i = 0; i < len; i++) {
			if (bytes[i] != 0) {
				return false;
			}
	...
	protected boolean isDiscardableFillerSegment(MemoryLoadable loadable, String blockName,
	...
	protected long getImageDataBase() {
	...
	protected AddressRange getMarkupMemoryRangeConstraint(Address addr) {
	 ...
	 */
	protected void processGNU(TaskMonitor monitor) throws CancelledException {
		monitor.checkCanceled();
...
//            }
	...
	protected void processEntryPoints(TaskMonitor monitor) throws CancelledException {
		monitor.checkCanceled();
	...
	protected void createDynamicEntryPoints(ElfDynamicType dynamicEntryType,
	...
	protected void markupInterpreter(TaskMonitor monitor) throws CancelledException {
		monitor.checkCanceled();
	...
	protected void processImports(TaskMonitor monitor) throws CancelledException {
		monitor.checkCanceled();
	...
	protected void processRelocations(TaskMonitor monitor) throws CancelledException {
	...
	protected void processRelocationTable(ElfRelocationTable relocationTable,
	 ...
	 * @param refAddr
	 */
	protected void addElfHeaderReferenceMarkup(int componentOrdinal, Address refAddr) {
	...
	protected void markupRelocationTable(Address relocTableAddr, ElfRelocationTable relocTable,
			TaskMonitor monitor) {
		try {
	...
	protected AddressSpace getDefaultDataSpace() {
		return program.getLanguage().getDefaultDataSpace();
	}

	protected AddressSpace getConstantSpace() {
		return program.getAddressFactory().getConstantSpace();
	}

	protected void allocateUndefinedSymbolData(HashMap<Address, Integer> dataAllocationMap) {
	...
	protected static final Integer AVAILABLE_MEMORY = 1;
	protected static final Integer ALLOCATED_MEMORY = 2;

	protected AddressSet allocatedRegions = new AddressSet();
	...
	protected AddressRange externalBlockLimits;
	protected Address lastExternalBlockEntryAddress;
	protected Address nextExternalBlockEntryAddress;
	 ...
	 */
	protected Address getNextExternalBlockEntryAddress(int entrySize) {
	 ...
	 */
	protected void createExternalBlock() {
	...
	protected void processSymbolTables(TaskMonitor monitor) throws CancelledException {
	...
	protected void processSymbols(ElfSymbol[] symbols, HashMap<Address, Integer> dataAllocationMap,
			TaskMonitor monitor) throws CancelledException {
	 ...
	 */
	protected Address findMemoryRegister(ElfSymbol elfSymbol) {
	...
	protected Address getMemoryRegister(String name, long value) {
	 ...
	 */
	protected Address allocateExternalSymbol(ElfSymbol elfSymbol) throws AddressOutOfBoundsException {
	 ...
	 */
	protected boolean processDuplicateExternal(ElfSymbol elfSymbol) {
		if (lastExternalBlockEntryAddress == null) {
			return false;
		}
		String symName = elfSymbol.getNameAsString();
	 ...
	 */
	protected boolean processVersionedExternal(ElfSymbol elfSymbol) {
	 ...
	 */
	protected Symbol findExternalBlockSymbol(String name, Address extMin, Address extMax) {
	...
	protected boolean isSymbolInRange(Symbol s, Address min, Address max) {
		Address symAddr = s.getAddress();
		return (symAddr.compareTo(min) >= 0 && symAddr.compareTo(max) <= 0);
	}

	protected void evaluateElfSymbol(ElfSymbol elfSymbol, Address address, boolean isFakeExternal)
	 ...
	 */
	protected boolean removeOldSymbol(Address address, String name) {
		SymbolTable symbolTable = program.getSymbolTable();
...
//	 */
//	protected AddressRange findFreeAddressRange(AddressSpace space) {
//
...
//	}
	...
	protected void markupGnuBuildId(TaskMonitor monitor) {
	...
	protected void markupGnuDebugLink(TaskMonitor monitor) {
	...
	protected void markupSymbolTable(Address symbolTableAddr, ElfSymbolTable symbolTable,
			TaskMonitor monitor) {
	 ...
	 * @throws InvalidInputException
	 */
	protected Address addDynamicMemoryReference(ElfDynamic elfDynamic, Data valueData,
	...
	protected void processStringTables(TaskMonitor monitor) throws CancelledException {
...
//					Long.toHexString(stringTable.getFileOffset()));
	...
	protected void markupStringTable(Address address, long tableBytesLength, TaskMonitor monitor) {
	...
	protected int createString(Address address)
	 ...
	 */
	protected AddressSpace getSegmentAddressSpace(ElfProgramHeader elfProgramHeader) {
	 ...
	 */
	protected AddressSpace getSectionAddressSpace(ElfSectionHeader elfSectionHeader) {
	 ...
	 */
	protected Address getSectionLoadAddress(ElfSectionHeader elfSectionHeader) {
...
// FIXME! Inconsistent in use of VirtualAddress which is generally an addressable word offset - not byte offset
	 ...
	 */
	protected Address findLoadAddress(long fileOffset, long headerSize) {
	 ...
	 */
	protected void processProgramHeader(ElfProgramHeader elfProgramHeader, int segmentNumber)
...
// process-specific extension should control the outcome.
	 ...
	 */
	protected long computeRelocationStartAddress(AddressSpace space, long baseOffset,
			TaskMonitor monitor) throws CancelledException {
		if (!elf.isRelocatable()) {
	 ...
	 * @throws AddressOutOfBoundsException if an invalid memory address is encountered
	 */
	protected void processSectionHeader(ElfSectionHeader elfSectionToLoad,
	...
	protected Data createUndefined(Address addr, int size) throws CodeUnitInsertionException {
	 ...
	 */
	protected Symbol checkPrimary(Symbol sym) {
	 ...
	 * @throws IOException
	 */
	protected InputStream getInitializedBlockInputStream(MemoryLoadable loadable, Address start,
	...
	protected String formatFloat(float value, int maxDecimalPlaces) {
	...
	protected long checkBlockLimit(String sectionName, long dataLength, boolean initialized)
			throws IOException {
	...
	protected class RelocatableImageBaseProvider {
