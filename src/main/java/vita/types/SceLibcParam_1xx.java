...
public class SceLibcParam_1xx implements StructConverter {
	...
	public SceLibcParam_1xx(ProcessingContext ctx, Address libcParamAddress, BinaryReader reader) throws IOException {
	...
	private void __markup_if_present(long address, String name) throws Exception {
		if (address != 0L)
			_ctx.api.createLabel(_ctx.textStart.getNewAddress(address), name, true, SourceType.ANALYSIS);
	}
}
